    ...
    <fieldset style="width: 71%">
        ...
        <form action="{{ route('sponsers.store') }}" method="POST" id="form1" style="display:none">
            ...
            <div class="idSection" style="margin-top: 15px">
                ...
                <input type="radio" name="ident_type" value="identification"
                    @if ($sponser->ident_type == 'identification') checked @endif />
                <label for="id">هوية</label>
                <input type="radio" name="ident_type" value="passport"
                    @if ($sponser->ident_type == 'passport') checked @endif />
                ...
                <input type="text" name="identifier" class="form-control @error('identifier') is-invalid @enderror"
                    id="identifier" value="{{ old('identifier', $sponser->identifier) }}" />
            </div>
            ...
            <div class="nameSection" style="margin-top: 15px">
                ...
                <input type="text" name="first_name" class="form-control @error('first_name') is-invalid @enderror"
                    value="{{ old('first_name', $sponser->first_Name) }}" />
                <input type="text" name="second_name" class="form-control @error('second_name') is-invalid @enderror"
                    value="{{ old('second_name', $sponser->second_name) }}" />
                <input type="text" name="third_name" class="form-control @error('third_name') is-invalid @enderror"
                    value="{{ old('third_name', $sponser->third_name) }}" />
                <input type="text" name="family_name" class="form-control @error('family_name') is-invalid @enderror"
                    value="{{ old('family_name', $sponser->family_name) }}" />
            </div>
            <div class="location" style="margin-top: 15px">
                ...
                <select name="country_id" id="country" class="form-control @error('country_id') is-invalid @enderror">
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" @if ($country->id == old('country_id', $sponser->country_id)) selected @endif>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="detailsOfLocation" style="margin-top: 15px">
                ...
                <input type="text" style="width: 850px" name="address"
                    class="form-control @error('address') is-invalid @enderror"
                    value="{{ old('address', $sponser->address) }}" />
            </div>
        ...
        ">
                <div>
                    ...
                    <input type="text" name="telephone"
                        ...
                        value="{{ old('telephone', $sponser->telephone) }}" />
                </div>
                <div>
                    <label for="">الجوال</label>
                    <input type="text" name="phone" class="form-control @error('phone') is-invalid @enderror"
                        value="{{ old('phone', $sponser->phone) }}" />
                </div>
                <div>
                    <label for="">البريد</label>
                    <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                        value="{{ old('email', $sponser->email) }}" />
                </div>
            </div>
            <div class="nationality" style="margin: 15px 0px">
                ...
                <select name="nationality" id="nationality"
                    ...
                    <option value="">...</option>
                    @foreach ($countries as $country)
                        <option value="{{ $country->name }}" @if ($country->name == old('nationality', $sponser->country_id)) selected @endif>
                            {{ $country->name }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <form action="{{ route('sponsers.store') }}" style="text-align: center" method="POST" id='form2'>
            ...
            <input type="text" id="name" name="name"
                ...
                value="{{ old('name', $sponser->name) }}"><br><br>

            <label for="responsibleOfContact">مسؤول الإتصال<label>
                    <input type="text" id="responsibleOfContact" name="responsible_name"
                        class="form-control @error('responsible_name') is-invaild @enderror"
                        value="{{ old('responsible_name', $sponser->responsible_name) }}"><br><br>

                    <label for="location">العنوان</label>
                    <input type="text" id="location" name="address"
                        class="form-controll @error('address') is-invalid @enderror"
                        value="{{ old('address', $sponser->address) }}"><br><br>

                    <div class="telephone">
                        <div class="tele1">
                            <label for="tele1">الهاتف1</label>
                            <input type="text" id="phone" name="phone"
                                class="form-controll @error('phone') is-invalid @enderror"
                                value="{{ old('phone', $sponser->phone) }}">

                        </div><br><br>
                        <div class="tele2">
                            <label for="tele2">الهاتف2</label>
                            <input type="text" id="tele2" name="phone2"
                                class="form-controll @error('phone2') is-invalid @enderror"
                                value="{{ old('phone2', $sponser->phone2) }}">

                        </div><br><br>
                        <label for="">البريد</label>
                        <input type="text" name="email" id=""
                            class="form-controll @error('email') is-invalid @enderror"
                            value="{{ old('email', $sponser->email) }}">
